/*
  ...
  * = very important; top priority
  ...
  - TODO: fix small bug when odd number of whitespaces on line and
          tab is pressed (resulting in an odd number whitespace tab
          depth) (TEMP FIX: disabled spacing when whitespace)
  - TODO: small bug when backspacing tab depth is not even and causes
          odd number whitespace bug
  - UPDATE: rewrote tabdepth to follow prev. line for tab depth (TODO
            : bug with tabdepth when prev line has on whitespace)
  - UPDATE: rewrote tab depth (now dependent on previous line tab 
           depth) (backspace to change tab depth not fixed)
           (COMPLETED)
  - UPDATE: odd number of whitespace, results in next line having
           previous line tab depth - 1
  ...
  - fixed minor bug for detecting mouse initiated cursor change when
    editor is empty
...
//NOTE: some of the descriptions for the css and js editors might not be listed, please see the corresponding htmleditor for comments
...
$(document).ready(function () {
  ...
  var htmleditor = CodeMirror.fromTextArea(document.getElementById("htmlcode"), {
    ...
    extraKeys: {
      Enter: function(cm) {
        ...
        var charInPrevLine = (htmleditor.getLine(htmleditor.getCursor().line)).split("");

        if(charInPrevLine.length != 0)
        {
          for (var i = 0; i < charInPrevLine.length; i++) {
            if(charInPrevLine[i] == " ")
            {
              spacesToAdd += " ";
              if(i % 2 == 0)
              {
                arr[arrIndex] = "htmleditor: @@tab∆" + (lengthToCursor + (i + 1));
                arrIndex++; 
              }
            }
            else
          ...
          if(spacesToAdd.length % 2 == 1)
          {
            spacesToAdd = spacesToAdd.substr(1);
          }
        }
  ...
  var csseditor = CodeMirror.fromTextArea(document.getElementById("csscode"), {
    ...
    extraKeys: {
      Enter: function(cm) {
        ...
        var charInPrevLine = (csseditor.getLine(csseditor.getCursor().line)).split("");
    ...
    onKeyEvent: function(editor, event){
      ...
      if(event.type == "keydown") {
        ...
        {
          //console.log(!hasWhiteSpace(csseditor.getLine(csseditor.getCursor().line)));
  ...
  var jseditor = CodeMirror.fromTextArea(document.getElementById("jscode"), {
    ...
    extraKeys: {
      Enter: function(cm) {
        ...
        var charInPrevLine = (jseditor.getLine(jseditor.getCursor().line)).split("");
    ...
    onKeyEvent: function(editor, event){
      ...
      if(event.type == "keydown") {
        ...
        {
          //console.log(!hasWhiteSpace(jseditor.getLine(jseditor.getCursor().line)));
  ...
  htmleditor.on("cursorActivity", function() {
    ...
    else if(htmleditor.getValue() == cursorPurpose && !htmleditor.somethingSelected() && htmleditor.getValue() != "")
    {
      if (arr[arrIndex-1].indexOf("@@arrow") == -1)
      {
  ...
  csseditor.on("cursorActivity", function() {
    ...
    else if(csseditor.getValue() == cursorPurpose && !csseditor.somethingSelected() && csseditor.getValue() != "")
    {
      if (arr[arrIndex-1].indexOf("@@arrow") == -1)
      {
  ...
  jseditor.on("cursorActivity", function() {
    ...
    else if(jseditor.getValue() == cursorPurpose && !jseditor.somethingSelected() && jseditor.getValue() != "")
    {
      if (arr[arrIndex-1].indexOf("@@arrow") == -1)
      {
